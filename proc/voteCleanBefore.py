...
# TODO: check district code consistent in base & cand
...
to_path = to_path + str(year) + "/" + votename + "/"
if not os.path.exists(to_path):
    os.makedirs(to_path)

id_match_df = pd.read_csv(id_match_path+matchfile, dtype = str).drop("voteID", axis = 1)
...
base_df = base_df.assign(trimVillage = base_df.nameVillage.str.slice(stop=3))
base_df = base_df.assign(names = lambda x: x.nameCity + x.nameTown + x.trimVillage)
base_df = pd.merge(base_df, id_match_df, how = "left")

oldid_df = base_df[["provinceID", "cityID", "districtID", "townID", "villageID", "id", "voteID", "oldID"]]

base_df = base_df[base_df.villageID != "0000"]
base_df = base_df.drop(["trimVillage", "names"], axis = 1)
base_df.to_csv(to_path+"elbase.csv", index = False)
...
cand_df = pd.merge(cand_raw_df, oldid_df, how = "left")
...
ckts_df = pd.merge(ckts_raw_df, oldid_df, how = "left")
...
prof_df = pd.merge(prof_raw_df, oldid_df, how = "left")
